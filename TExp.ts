...
import { Result, bind, makeOk, makeFailure, mapResult, mapv, either, isFailure } from "../shared/result";
...
const allSimpleAtomics = [makeNumTExp(), makeBoolTExp(), makeStrTExp(), makeVoidTExp()]
...
const normalizeUnion = (ute: UnionTExp): TExp =>
    ...
    includes(makeAnyTExp(), ute.components) ? makeAnyTExp() : //if includes TExp of type Any, then the return TExp is Any
    (ute.components.length === 1) ? ute.components[0] : //if has only one component to union, then returns it
    ute; //otherwise, meaning this is a regular case of union, returns UnionTExp
...
const normalizeInter = (ute: InterTExp): TExp =>
    ...
    (includes(makeAnyTExp(), ute.components) && ute.components.length>1) ? makeInterTExp(filter((x)=>!isAnyTExp(x),ute.components)) : //remove any types
    (includes(makeAnyTExp(), ute.components) && ute.components.length==1) ? makeNeverTExp() : 
    (ute.components.length === 1) ? ute.components[0] : 
    (ute.components.length === 0) ? makeNeverTExp() : 
    ute;
...
const makeDiffTExp = (te1: TExp, te2: TExp): TExp => {
    ...
    if (isUnionTExp(te1) || isInterTExp(te1)){ //Case 1: te1 is union or intersection
        if (!isUnionTExp(te2) && !isInterTExp(te2)){ //Case 1.a: te2 is atomic type
            ...
            if(filteredList.length==0) return makeNeverTExp();
            if(filteredList.length==1) return filteredList[0];
            return makeUnionTExp(filteredList); //reach only if there are 2 or more types
        }
        else{ //Case 1.b: te2 is also union or intersection
            const te2Tags = map((x:TExp)=>x.tag,te2.components)
            const filteredList = filter((x:TExp)=> !includes(x.tag, te2Tags), te1.components);
            if(filteredList.length==0) return makeNeverTExp();
            if(filteredList.length==1) return filteredList[0];
            return makeUnionTExp(filteredList); //reach only if there are 2 or more types
        }
        ...
        else{ //Case 2.b: te2 is union or intersection
            const te2Tags = map((x:TExp)=>x.tag,te2.components)
            if(isAnyTExp(te1)){
                const filteredList = filter((x:TExp)=> !includes(x.tag,te2Tags), allSimpleAtomics);
                return makeUnionTExp(filteredList); //must be of length 3
            }
            if (includes(te1.tag, te2Tags)) return makeNeverTExp();
            return te1;
        }
    }
}
...
// Purpose: uniform access to atomic types' tag
...
export const parseTE = (t: string): Result<TExp> =>
    bind(p(t), parseTExp); //p is the parser, appears above as: import { parse as p } from "../shared/parser";
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    isString(texp) ? makeOk(makeTVar(texp)) : //is type variable
...
*/
export const unparseTExp = (te: TExp): Result<string> => { //receives a type expression, returns Result with a val of string
