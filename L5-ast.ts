...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, isTypePredTExp } from './TExp';
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | TypePredExp;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isTypePredExp(x);
...
export type TypePredExp = { tag: "TypePredExp", predTE: TExp}
export const makeTypePredExp = (predTE: TExp): CExp => 
    ({ tag: "TypePredExp", predTE: predTE })
...
const makePred = (param: TExp): CExp[] => {
    return [makeTypePredExp(param)];
}
